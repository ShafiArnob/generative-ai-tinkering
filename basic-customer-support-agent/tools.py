...
def data_protection_check(
    name: str, postcode: str, year_of_birth: int, month_of_birth: int, day_of_birth: int
) -> Dict:
    """
    ...
    for customer in customers_database:
        if (
            ...
            and customer["postcode"].lower() == postcode.lower()
            and int(customer["dob"][0:4]) == year_of_birth
            and int(customer["dob"][5:7]) == month_of_birth
            and int(customer["dob"][8:10]) == day_of_birth
        ):
...
@tool
def retrieve_exixting_customer_orders(customer_id: str) -> List[Dict]:
    """
...
def place_order(items: Dict[str, int], customer_id: str) -> str:
    ...
    # check if the quantities of items are valid
    availability_messages = []
    valid_item_ids = [item["id"] for item in inventory_database]
    for item_id, quantity in items.items():
        if item_id not in valid_item_ids:
            availability_messages.append(
                f"Item with id {item_id} is not found in the inventory"
            )
        else:
            inventory_item = [
                item for item in inventory_database if item["id"] == item_id
            ][0]
            if quantity > inventory_item["quantity"]:
                availability_messages.append(
                    f'There is insufficient quantity in the inventory for this item {inventory_item["name"]}\nAvailable: {inventory_item["quantity"]}\nRequested: {quantity}'
                )
